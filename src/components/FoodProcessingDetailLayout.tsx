      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 xl:gap-16">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="glass-panel p-5 sm:p-6 rounded-2xl border border-light-300 flex flex-col gap-1 shadow-xs bg-gradient-to-br from-white to-blue-50/30">
            ...
            {activeVariant?.applicableGst && (
              <p className="text-xs text-dark-500 font-medium">
                Inclusive of {activeVariant.applicableGst}% GST. Precision mesh sieves & motor pulley included.
              </p>
            )}
          </div>
                ...
                {variants.map((v) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-md border-2 text-left transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${
                        isSelectedVariant
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2">
            <Link
              href="/dealers"
              className="flex-1 bg-blue-700 hover:bg-blue-800 text-white font-bold py-4 px-6 rounded-sm shadow-md transition-all flex items-center justify-center gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 min-h-[48px]"
            >
              <MapPin className="w-5 h-5" />
              Locate Commercial Processing Dealer
            </Link>
            <label
              className={`flex-none flex items-center justify-center gap-2 px-5 py-4 border-2 border-light-300 rounded-sm hover:border-dark-500 transition-colors group bg-white focus-within:ring-2 focus-within:ring-blue-500 min-h-[48px] ${
            ...
            >
              <input
                type="checkbox"
                className="w-5 h-5 border-gray-300 rounded-sm text-blue-600 focus:ring-blue-500 disabled:cursor-not-allowed"
                checked={mounted ? isSelected : false}
                onChange={handleCompareChange}
                disabled={disabled}
              />
              <span className={`text-xs font-bold ${isSelected ? "text-blue-700" : "text-dark-900"}`}>
