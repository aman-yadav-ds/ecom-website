            ...
            {displayedProducts.map((product) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className="relative w-full h-[240px] md:h-[260px] bg-white p-6 flex items-center justify-center overflow-hidden border-b border-light-200">
                      <div className="absolute top-4 left-4 z-10 flex items-center gap-1.5 glass-panel text-dark-900 text-[10px] md:text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider border border-light-300 shadow-xs">
                    ...
                    <div className="p-6 flex flex-col flex-grow bg-white/80 backdrop-blur-md">
                      <span className="text-[11px] font-bold text-brand-red uppercase tracking-widest mb-1.5">
                        {product.tags[0] || "Machinery"}
                      </span>
                      <h3 className="text-dark-900 text-lg md:text-xl font-bold uppercase mb-2 group-hover:text-brand-red transition-colors duration-200 line-clamp-1">
                        {product.name}
                      </h3>
                      <p className="text-dark-600 text-xs md:text-sm leading-relaxed line-clamp-2 mb-4 font-medium">
                        {product.description}
                      </p>

                      <div className="mt-auto pt-4 border-t border-light-300 flex items-center justify-between">
                        <span className="text-lg md:text-xl font-extrabold text-dark-900">
                          {price > 0 ? priceFormatter.format(price) : "Price on Request"}
                        </span>
                        <div className="flex items-center gap-1 text-brand-red font-bold text-xs uppercase tracking-wider group-hover:translate-x-1 transition-transform">
                          <span>View Spec</span>
