...
const featuredProducts = [
  ...
  },
  {
    ...
    image: "/products/brushcutter.webp", // From public/products
    ...
    <section className="bg-light-200 py-16 md:py-24 px-4 border-b border-light-300">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-12 gap-6">
          <ScrollReveal animation="slide-left" className="w-full">
          ...
          <ScrollReveal animation="slide-right" className="hidden md:block">
            <Link 
              href="/products" 
              className="flex items-center gap-2 text-brand-red border border-brand-red hover:bg-brand-red hover:text-white font-semibold text-sm transition-all duration-300 px-6 py-2.5 rounded-full group whitespace-nowrap"
            >
              VIEW ALL PRODUCTS
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
          {featuredProducts.map((product, index) => (
            <ScrollReveal key={index} animation="slide-bottom" delay={index * 150}>
              <Link 
                href={product.link}
                className="group block bg-white rounded-xl shadow-sm hover:shadow-2xl transition-all duration-300 overflow-hidden border border-light-300 hover:border-brand-red h-full"
              >
        ...
        <ScrollReveal animation="fade" className="mt-8 md:mt-10 flex justify-center md:hidden">
          <Link 
            href="/products" 
